...
│   ├── profile.js         步骤01：绩点档案（表单、课程卡片渲染、当前状态总览同步）
│   ├── history.js         步骤02上半：历史成绩（手动填写 + 拖拽图片导入）
│   ├── ability.js         步骤02下半：学习能力自评 + AI 能力诊断
│   ├── plan.js            步骤03：个性化选课建议（AI 生成）
│   ├── simulator.js       步骤04：成绩模拟计算器（滑块、实时计算）
...
1. **绩点档案** —— 本学期课程 + 评分规则（含截图导入），末尾附"当前状态总览"
2. **能力诊断** —— 历史成绩（拖拽图片 / 手动填写，可跳过）+ 能力自评，点按钮生成 AI 诊断
3. **个性化选课建议** —— 每周时间 / 优先目标 / 是否有实习竞赛，点按钮生成 AI 建议
4. **成绩模拟计算器** —— 拖滑块看不同发挥下的总评和绩点
5. **学习方案** —— 根据绩点档案自动算出的优先级排序，不需要额外点按钮
