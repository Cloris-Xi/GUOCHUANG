...
"历史均绩"模块（大一新生可以直接跳过）有三种录入方式，可以混用：

- **手动填"目前绩点"**：如果你已经在教务系统查到了自己的累计绩点，
  直接填 `manualCurrentGpa` 这个输入框最省事，不用逐门课去凑。填了
  之后，`ability-diagnosis` / `course-advice` / `study-plan` 三个
  AI 接口都会优先参考这个数字，而不是靠下面逐门课算出来的平均值。
...
- **逐门手动填写**：点"+ 手动添加一门历史课程"，自己填名称/学分/
  成绩/类型标签，页面会实时算出学分加权的平均绩点，作为没填
  `manualCurrentGpa` 时的参考。
