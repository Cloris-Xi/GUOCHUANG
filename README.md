...
1. **绩点档案** —— 内部分三小块：
   1. 绩点制度（4.0/4.3/5.0 三种常见制式，或选"自定义"自己填换算表）
   2. 历史均绩（可选：手动填"目前绩点"最省事；也可以拖拽成绩单截图导入 / 逐门手动填写，会实时算出学分加权的历史平均绩点作参考）
...
"目前绩点"（`manualCurrentGpa`）是**必填项**——除非勾选"我是大一
新生，还没有绩点"（`isFreshman`），勾了之后输入框会被禁用并清空，
三个 AI 触发按钮（能力诊断、AI 选课建议、生成学习方案）在点击时都
会先检查 `isCurrentGpaProvided()`（定义在 `js/history.js` 里），
没填且没勾新生选项会拦下来提示先去填，不会带着空值调用模型。

"历史均绩"模块除了"目前绩点"，还支持两种可选的录入方式，可以混用：
...
- **拖拽导入**：把成绩单/教务系统截图拖进那个虚线框，或者点击选择图片，
  ...
  已经手动填好的内容。
- **逐门手动填写**：点"+ 手动添加一门历史课程"，自己填名称/学分/
  成绩/类型标签，页面会实时算出学分加权的平均绩点，作为参考——但
  真正传给 AI 的是"目前绩点"这个手动填的数字，逐门算出来的平均值
  只在页面上展示，不会覆盖它。

"AI 能力与方向诊断"模块把"历史均绩"这些数据 + "学习能力自评"
...
"绩点制度"下拉框选"自定义"后会出现一个可编辑的表格（和评分构成表格
是同一套交互）：填"多少分以上算多少绩点"，比如 `90 → 4.0`、
`80 → 3.0`，不用自己排序，`js/profile.js` 里的 `rebuildCustomScaleTable()`
会在每次修改后自动按分数从高到低重排，并且保证兜底有一档 `0分 → 0绩点`，
避免出现某个分数区间没有对应绩点、算出 `undefined` 的情况。
